...
<body>
	<table id="ErrMsgTable">
		...
		<tr>
			...
			<br>
			<font class='ErrMsgFont' id="EMSG"><br><fmt:message bundle="${messages_directory}" key="directory.error.title.systemError" />
			<br>
			<c:choose>
			<c:when test="${errorCode == 5006}"><fmt:message bundle="${messages_directory}" key="directory.error.message.5006.notAuthorizedUser" /></c:when>
			<c:otherwise><fmt:message bundle="${messages_directory}" key="directory.error.message.3000.errorMessage" /></c:otherwise>
			</c:choose>
			</font>
			</td>
			<td width='10'>&nbsp;&nbsp;</td>
